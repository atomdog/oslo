...
#communications
def getMail():
    mail = gmailfunctions.readMail()
    return(mail)
def sendTextMail(message, endpoint):
    return(gmailfunctions.sendTextMail(message,endpoint))
#end communications
...
def latlonglookup(lat,long):
    ...
    #126 Browne Street,Brookline,MA,02446,United States
    addressfirst = address.split(",")
    statezipsplit = addressfirst[2].split(" ")
    print(statezipsplit)
    faddress = addressfirst[0], addressfirst[1].replace(" ", ""), statezipsplit[1].replace(" ", ""), statezipsplit[2].replace(" ", ""), addressfirst[3].replace(" ", "")
    return(faddress)
